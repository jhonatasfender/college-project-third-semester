...
class ImageController extends Controller
{
     ...
     *
     * @param  string  $path
     * @param  string  $options Exemple w_23,h_23
     * @param  string  $name
    ...
    public function show($path, $options, $name, $extension)
    {
        ...
        try {
            $imageNew = $this->imageManager->make($imagePath);
    ...
    {
        return $this->getDisk()->exists($imageSmallPath) &&
        $this->getDisk()->lastModified($imagePath) <= $this->getDisk()->lastModified(
            $imageSmallPath
        );
    }
    ...
    protected function getPath($path)
    {
        ...
        return storage_path(trim(str_replace($from, '', $path), '/'));
    }
